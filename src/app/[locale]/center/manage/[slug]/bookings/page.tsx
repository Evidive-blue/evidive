...
export default async function CenterManageBookingsPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string; slug: string }>;
    ...
    <div className="pt-8 pb-16">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 flex items-center justify-between text-sm text-white/60">
          <span>{t("results", { count: totalCount })}</span>
          {totalPages > 1 && (
          ...
          {typedBookings.length === 0 ? (
            ...
            typedBookings.map((booking) => (
              <CenterBookingCard
                ...
                locale={locale}
                detailsHref={`${basePath}/bookings/${booking.id}`}
                translations={{
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-center gap-2">
            {currentPage > 1 && (
              <Link
                href={`${basePath}/bookings?${new URLSearchParams({
                  ...filters,
