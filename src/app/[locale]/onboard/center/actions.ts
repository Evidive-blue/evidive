...
/**
 * Create an additional center for an existing CENTER_OWNER
 * Uses the current logged-in user's account info
 */
export async function createAdditionalCenter(
  centerInfo: {
    centerName: string;
    description: string;
    website?: string;
  ...
  try {
    ...
    if (!profile || !profile.email) {
      return { success: false, errorCode: "CENTER_REGISTER_FAILED", error: "User not found" };
    }

    // Update user type to CENTER_OWNER if not already
    if (profile.userType !== "CENTER_OWNER" && profile.userType !== "ADMIN") {
      await prisma.profile.update({
        where: { id: profile.id },
        data: { userType: "CENTER_OWNER" },
      });
    }
    ...
    const center = await prisma.diveCenter.create({
      data: {
        ...
        longitude: locationParsed.data.longitude,
        email: profile.email,
        phone: (centerInfo.centerPhone || profile.phone || "").trim(),
        website: centerInfo.website || null,
        facebook: centerInfo.facebook || null,
        instagram: centerInfo.instagram || null,
        status: "PENDING",
      ...
      try {
        await sendEmail({
          ...
          text: [
            ...
            `Nom: ${centerInfo.centerName}`,
            `Email: ${profile.email}`,
            `Propriétaire: ${profile.displayName || "N/A"}`,
            `Ville: ${locationData.city}`,
            `Pays: ${locationData.country}`,
            `CenterId: ${center.id}`,
          ].join("\n"),
    ...
    console.error("Error creating additional center:", error);
    return {
      success: false,
      errorCode: "CENTER_REGISTER_FAILED",
      error: "Failed to create additional center",
    };
  }
}
